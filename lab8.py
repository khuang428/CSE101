...
def search_car(query, dealerships):  # SELECT * FROM dealership1 WHERE brand == Ford AND paint == RED
    q = query.split()
    ...
    if dealer == None:
        return None
    for car in dealer.car_list:
        if attr == 'id':
            if car.id == attrVal:
                cars.append(car)
        elif attr == 'brand':
            if car.brand == attrVal:
                cars.append(car)
        elif attr == 'price':
            if car.price == int(attrVal):
                cars.append(car)
        elif attr == 'paint':
            if car.attributes.paint == attrVal:
                cars.append(car)
        elif attr == 'tires':
            if car.attributes.tires == attrVal:
                cars.append(car)
        elif attr == 'trim':
            if car.attributes.trim == attrVal:
                cars.append(car)
    return cars
